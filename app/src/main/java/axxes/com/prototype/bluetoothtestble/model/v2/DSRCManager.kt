...
class DSRCManager {
    ...
    fun prepareGNSSPacket(timestamp: Long, longitude: Int, latitude: Int, hdop: Int, numberOfSattelites: Int): ByteArray{
        ...
        ret += toBytes(timestamp,4)
        ret += toBytes(longitude,4)
        ret += toBytes(latitude,4)

        ret += toBytes(hdop,1)
        ret += toBytes(numberOfSattelites,1)
    ...
    fun readGNSSPacket(gnssPacket: ByteArray): String{
        ...
        val ite = gnssPacket.iterator()
        var currByte: Byte
        var text = "Response : \n"
        var l: Int
        var n = 0
        while(ite.hasNext() && n != parameters.size){
            Log.d("DSRCManager","$n ieme paramètre : ${parameters[n].first}")
